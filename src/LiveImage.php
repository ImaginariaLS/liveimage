...
class LiveImage implements LiveImageInterface
{
    /**
    ...
    {
        if (!$file || !($image_properties = getimagesize($file))) {
            $this->set_last_error(3);
            throw new LiveImageException( $this->error_messages[3], 3 );
        }
        
        /**
         * Определяем тип файла изображения
         */
        switch ($image_properties['mime']) {
            case 'image/png':
            case "image/x-png":
            ...
            default:
                ...
                throw new LiveImageException( $this->error_messages[5], 5);
        }
        /**
        ...
        if (!$tmp) {
            ...
            throw new LiveImageException( $this->error_messages[4], 4);
        }

        $this->image = $tmp;
        $this->width = $image_properties[0];
        $this->height = $image_properties[1];
        $this->truecolor = true;
    }
    ...
    public function resize($width = null, $height = null, bool $scale = false, bool $alfa = true)
    {
        ...
        if ($this->format === 'gif') {
            ...
            if (!function_exists($sResizeFunction)) {
                $sResizeFunction = 'imagecopyresized';
            }
            if (isset($ct) and $ct != -1) {
                $sResizeFunction = 'imagecopyresized';
            }
    ...
    public function cropSquare($bCenter = true)
    {
        $iWidth = $this->getImageParams( 'width' );
        $iHeight = $this->getImageParams( 'height' );
        ...
        if ($bCenter) {
            return $this->crop( $iNewSize, $iNewSize, ($iWidth - $iNewSize) / 2, ($iHeight - $iNewSize) / 2 );
        } else {
            return $this->crop( $iNewSize, $iNewSize, 0, 0 );
        }
    }
    ...
    public function cropProportion(LiveImage $oImage, $iW, $iH, $bCenter = true)
    {
        ...
        if ($bCenter) {
            $this->crop( $iNewWidth, $iNewHeight, ($iWidth - $iNewWidth) / 2, ($iHeight - $iNewHeight) / 2 );
        } else {
            $this->crop( $iNewWidth, $iNewHeight, 0, 0 );
        }
    ...
    public function setImage($image_res)
    {
        if ((int)@imagesx( $image_res ) > 0) {
    ...
    {
        return $this->setImage($image_res);
    }
    
    public function getImageParams($key = null)
    {
        $params = [
            'width' => $this->width,
            'height' => $this->height,
    ...
    {
        return $this->getImageParams($key);
    }

    public function setFont($font_size = 20, $font_angle = 0, $name = '')
    {
        if ($name) {
            $this->font = $name;
        }
    ...
    {
        $this->setFont($font_size, $font_angle, $name);
    }

    public function setColor($r = 255, $g = 255, $b = 255, $transparency = false)
    {
    ...
    {
        $this->setColor($r, $g, $b, $transparency);
    }
    ...
    protected function to_unicode($text, $from = 'w')
    {
        ...
        $text = mb_convert_encoding($text, 'iso8859-5', 'windows-1251');
